...
func (m *MongoStorage) GetUser(userID string) (user *vsec.User, err error) {
	...
	return user, vmgo.LogError("UMan:Mongo", err)
}
...
func (m *MongoStorage) GetUsers(offset, limit int, filter *vcmn.Filter) (
	...
	err = conn.C("users").
		...
		Limit(limit).
		All(&users)
	return users, vmgo.LogError("UMan:Mongo", err)
}
...
func (m *MongoStorage) GetCount(filter *vcmn.Filter) (count int, err error) {
	...
	return count, vmgo.LogError("UMan:Mongo", err)
}
...
// 	}
// 	return total, users, vmgo.LogError("UMan:Mongo", err)
// }
